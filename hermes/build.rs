...
fn main() {
    ...
    // generate common templates loader
    generate_common_templates_loader(&manifests_root, &out_dir);
}
...
/// generate code to load common templates
fn generate_common_templates_loader(manifests_root: &Path, out_dir: &Path) {
    let common_dir = manifests_root.join("_common");
    let output_file = out_dir.join("common_templates.rs");

    let mut code = String::new();
    code.push_str("// auto-generated by build.rs\n\n");
    code.push_str("pub fn load_common_templates(env: &mut minijinja::Environment) {\n");

    if common_dir.exists()
        && let Ok(entries) = fs::read_dir(&common_dir)
    {
        let mut template_files: Vec<_> = entries
            .filter_map(|e| e.ok())
            .filter(|e| e.path().extension().is_some_and(|ext| ext == "j2"))
            .collect();

        template_files.sort_by_key(|e| e.path());
        ...
        for entry in template_files {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                ...
                code.push_str(&format!(
                    "    env.add_template(\n        \"{}\",\n        include_str!(\"{}\"),\n    ).expect(\"failed to load common template: {}\");\n\n",
                    template_name, absolute_path, template_name
                ));
            }
        }
    }
    ...
    fs::write(&output_file, code).expect("failed to write common templates code");
    eprintln!("Generated common templates loader at {:?}", output_file);
}
