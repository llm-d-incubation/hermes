...
/// Header info for generated manifests (dry-run mode)
pub struct ManifestHeader<'a> {
    pub workload_name: &'a str,
    pub config: &'a SelfTestConfig,
    pub rdma_info: &'a workloads::RdmaInfo,
    pub node_pair: &'a NodePair,
}

impl<'a> std::fmt::Display for ManifestHeader<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "# Auto-generated by Hermes (RDMA Cluster Analyzer)")?;
        writeln!(f, "# https://github.com/llm-d-incubation/hermes")?;
        writeln!(f, "#")?;
        writeln!(f, "# Configuration:")?;
        writeln!(f, "#   workload: {}", self.workload_name)?;
        writeln!(f, "#   namespace: {}", self.config.namespace)?;
        writeln!(f, "#   image: {}", self.config.image)?;

        if let Some(ref sriov_net) = self.rdma_info.sriov_network {
            writeln!(f, "#   sriov_network: {}", sriov_net)?;
        }

        if let Some(gpus) = self.config.gpus_per_node {
            writeln!(f, "#   gpus_per_node: {}", gpus)?;
        }
        ...
        if !self.rdma_info.ucx_gid_index.is_empty() {
            writeln!(f, "#   ucx_gid_index: {}", self.rdma_info.ucx_gid_index)?;
        }

        if let Some(ref rule) = self.config.topology_rule {
            writeln!(f, "#   topology_rule: {}", rule)?;
        }

        writeln!(f, "#")?;
        writeln!(f, "# Selected nodes:")?;
        writeln!(f, "#   server_node: {}", self.node_pair.node1.name)?;
        writeln!(f, "#   client_node: {}", self.node_pair.node2.name)?;
        writeln!(
            f,
            "#   selection_reason: {}",
            self.node_pair.selection_reason
        )?;
        writeln!(f, "#")?;
        Ok(())
    }
...
impl SelfTestOrchestrator {
    ...
    async fn cleanup_resources_static(
        ...
        if let Ok(svc_list) = services.list(&lp).await {
            for svc in svc_list.items {
                ...
                {
                    tracing::debug!("Failed to delete service {}: {}", svc_name, e);
                }
            }
        }
        ...
        if let Ok(cm_list) = configmaps.list(&lp).await {
            for cm in cm_list.items {
                if let Some(cm_name) = cm.metadata.name
                    && let Err(e) = configmaps.delete(&cm_name, &DeleteParams::default()).await
                {
    ...
    pub async fn run(&self) -> Result<TestExecution> {
        ...
        info!("Analyzing cluster for optimal RDMA node pairing...");
        ...
        info!("Reason: {}", node_pair.selection_reason);
        ...
        info!("Deploying test workload: {}", workload.name());
    ...
    async fn execute_test(
        ...
        info!("Preparing test workload: {}", workload.name());
        info!("Description: {}", workload.description());
    ...
    async fn render_manifests_to_stdout(
        ...
        let header = ManifestHeader {
            workload_name: &test_execution.workload_name,
            config: &self.config,
            rdma_info: &rdma_info,
            node_pair: &test_execution.node_pair,
        };
        println!("{}", header);
