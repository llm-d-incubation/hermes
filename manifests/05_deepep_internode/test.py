...
def test_main(args: argparse.Namespace,
    ...
    # Modified assertion to support 1, 2, 4, or 8 local ranks
    assert num_experts % num_ranks == 0, f"num_experts ({num_experts}) must be divisible by num_ranks ({num_ranks})"
    assert num_local_ranks in [1, 2, 4, 8], f"num_local_ranks must be 1, 2, 4, or 8, got {num_local_ranks}"

    # Ensure we have enough ranks for multi-node testing if specified
    if num_nodes > 1:
        assert num_ranks >= num_local_ranks * 2, f"Need at least {num_local_ranks * 2} ranks for multi-node testing"

    if local_rank == 0:
        print(f'[config] num_tokens={num_tokens}, hidden={hidden}, num_topk_groups={num_topk_groups}, num_topk={num_topk}', flush=True)
        print(f'[config] num_local_ranks={num_local_ranks}, num_ranks={num_ranks}, num_nodes={num_nodes}', flush=True)
    ...
    # Config - adjust buffer sizes based on number of local ranks
    # Scale buffer sizes proportionally to the number of GPUs per node
    base_rdma_buffer = 128
    base_nvl_buffer = 720 if num_ranks in (24, 48, 96, 144, 160) else 512

    # Adjust buffer sizes for smaller GPU counts
    rdma_buffer_size = max(32, base_rdma_buffer * num_local_ranks // 8)
    nvl_buffer_size = max(64, base_nvl_buffer * num_local_ranks // 8)

    # Adjust chunk size limits based on local ranks
    nvl_chunk_size = min(8, max(2, num_local_ranks))
    rdma_chunk_size = 16
    ...
    # Tune dispatch performance - adjust ranges based on num_local_ranks
    best_dispatch_results = None
    fp8_factor = (1 + 4 / 128) / 2

    # Adjust tuning ranges based on number of local ranks
    max_nvl_chunk = min(45, max(8, num_local_ranks * 6))
    max_rdma_chunk = min(33, max(8, num_local_ranks * 4))
    ...
    for current_x in (x_e4m3, x):
        ...
        for nvl_chunk_size in range(4, max_nvl_chunk, 4):
            for rdma_chunk_size in range(4, max_rdma_chunk, 4):
                config = deep_ep.Config(num_sms, nvl_chunk_size, nvl_buffer_size, rdma_chunk_size, rdma_buffer_size)
    ...
    # Tune combine performance - adjust ranges
    max_combine_nvl_chunk = min(8, max(2, num_local_ranks))
    min_combine_rdma_chunk = 12 if num_nodes == 2 and num_local_ranks >= 4 else 8

    best_time, best_results = 1e10, None
    for nvl_chunk_size in range(1, max_combine_nvl_chunk, 1):
        for rdma_chunk_size in range(min_combine_rdma_chunk, max_rdma_chunk, 4):
            config = deep_ep.Config(num_sms, nvl_chunk_size, nvl_buffer_size, rdma_chunk_size, rdma_buffer_size)
...
def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    ...
    # Only test LL compatibility if we have 8 local ranks (original requirement)
    if args.test_ll_compatibility and num_local_ranks != 8:
        if local_rank == 0:
            print(f"WARNING: Low-latency compatibility test requires 8 local ranks, skipping (have {num_local_ranks})")
    ...
    # Modified assertion to support flexible local ranks
    assert num_local_ranks in [1, 2, 4, 8], f"num_local_ranks must be 1, 2, 4, or 8, got {num_local_ranks}"

    # For pure transport testing, we need at least 2 nodes worth of ranks
    min_ranks = num_local_ranks * 2
    if num_ranks < min_ranks:
        if local_rank == 0:
            print(f"WARNING: Testing with {num_ranks} ranks may not fully exercise inter-node communication")
            print(f"         Recommended minimum: {min_ranks} ranks for {num_local_ranks} local ranks")
    ...
    # Test compatibility with low latency functions (only if 8 local ranks)
    if args.test_ll_compatibility:
...
if __name__ == '__main__':
    ...
    # Validate num_processes is 1, 2, 4, or 8
    if num_processes not in [1, 2, 4, 8]:
        print(f"ERROR: --num-processes must be 1, 2, 4, or 8 (got {num_processes})")
        print("       This represents the number of GPUs per node")
        exit(1)
